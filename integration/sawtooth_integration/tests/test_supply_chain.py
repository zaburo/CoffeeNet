...
class TestSupplyChain(unittest.TestCase):
    ...
    def test_track_and_trade(self):
        ...
        self.narrate(
            '''
            Jin tries to create a record for a coffee with label `coffee-456`.
            ''')
        ...
        self.narrate(
            '''
            He said his coffee record should have type `coffee`, but that
            type doesn't exist yet. He needs to create the `coffee`
            record type first.
            ''')

        self.narrate(
            '''
            Jin creates the record type `coffee` with properties {}.
        ...
        self.narrate(
            '''
            Now that the `coffee` record type is created, Jin can create
            his coffee record.
            ''')
        ...
        self.narrate(
            '''
            Jin updates the coffee record's temperature. Updates, of
        ...
        self.narrate(
            '''
            Jin gets tired of sending coffee updates himself, so he decides to
        ...
        self.narrate(
            '''
            Jin would like to sell his coffee to Sun, a coffee dealer. Of
        ...
        self.assert_invalid(
            sun.finalize_record('coffee-456'))

        self.narrate(
            '''
            In order to finalize a record, the owner and the custodian
            must be the same person. Sun is the owner of the coffee in a
            legal sense, but Jin is still the custodian (that is, he
            has physical custody of it). Jin hands over the coffee.
            ''')
        ...
        self.assertEqual(get_record_property['recordId'], 'coffee-456')
        ...
        get_record = jin.get_record('coffee-456')
